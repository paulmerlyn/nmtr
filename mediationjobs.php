<?php
/*
This mediation jobs page (sister to arbitrationjobs.php) requires a small micropayment "donation" from the page visitor. It exploits PayPal's new micropayment fee schedule set up on my personal (sole prop) name@yahoo.com PayPal account. The PHP code tests whether the visitor made a Completed donation via PayPal (or, alternatively, was referred from the MediationCareer.org server). If he/she did, he/she is then shown the full list of mediation jobs. If the PHP code isn't able to detect the Completed payment status in the returned URL query string, then the page is shown with a large black screen div that requests a donation via a PayPal "Buy Now" button.
*/

// Start a session
session_start();

// I use this technique (courtesy: http://celestial-star.net/tutorials/49-check-referrer/) so that I can see the mediationjobs.php page myself without the overlay screen and without my having to make a donation! It works by permitting the page to show if the $_SERVER['HTTP_REFERER']'s path is the jobspagestest.html private page).
...
// As a second check of whether the visitor should see the page without the overlay screen, set the $validDonationFlag to true if the visitor is me via the jobspagestest.html page.
...
<title>Mediation Jobs</title>
<meta NAME="description" CONTENT="Listing of mediation jobs and jobs in the conflict resolution field, updated daily.">
<meta NAME="keywords" CONTENT="mediation jobs, jobs for mediators, jobs in conflict resolution">
<!-- InstanceEndEditable -->
...
function clicker(){
	...
	if(thediv.style.display == "none"){
		...
		thediv.innerHTML = "<table width='100%' height='100%'><tr><td align='center' valign='middle' width='100%' height='100%'><img alt='mediation jobs donation' width='256' height='173' style='position: relative; bottom: 20px;' src='/images/mediationjobsdonation.jpg'><p>The Mediation Jobs board currently lists approximately 75 recently posted jobs for mediators or applicants with conflict resolution skills. In order to view this page, please make a donation &mdash; minimum $1.00, please &mdash; to the National Mediation Training Registry. Donations are an important source of financial support and help to defray operating costs.</p><p>Good luck as you explore jobs in the rapidly advancing field of mediation, and thanks for supporting Alternative Dispute Resolution through your donation!</p><form action='https://www.paypal.com/cgi-bin/webscr' method='post'><input type='hidden' name='cmd' value='_s-xclick'><input type='hidden' name='hosted_button_id' value='GTFBUGQTZ4US8'><input type='hidden' name='page_style' value='NMTR'><input type='image' src='https://www.paypal.com/en_US/i/btn/btn_donate_LG.gif' border='0' name='submit' alt='PayPal - The safer, easier way to pay online!'><img alt='' border='0' src='https://www.paypal.com/en_US/i/scr/pixel.gif' width='1' height='1'></form></td></tr></table>";
	}else{
...
<h1>Mediation Jobs</h1>
<p>A compilation of recently posted mediator jobs and associated jobs in the mediation field:</p>
...
$response = callAPIQuick('http://api.simplyhired.com/a/jobs-api/xml-v2/q-mediation+OR+mediator+AND+NOT+%28+%22postdoctoral+fellow%22+OR+%22site+operation%%22validation+manager%22+OR+%22product+analyst%22+OR+cardiology+OR+%22Mill+Manager%22+OR+voip+OR+sdlc+OR+%22Customer+Care+Representative%22+OR+Creative+OR+%22Graduate+assistantship%22+OR+%22Doctoral+Fellow%22+OR+%22mediate+risk%22+%29+AND+NOT+%28C+OR+C%2B%%29+AND+NOT+%28title%3A%28+mitigation+OR+developer+OR+engineering+OR+%22Customer+Service+Representative%22+OR+programmer+OR+Servicing+OR+unit+OR+architect+OR+assistantship+OR+Pyk2+OR+estrogen+OR+fellow+OR+%22business+coordinator%22+%22medical+assistant%22+underwriting+OR+underwriter+OR+creative+OR+Maintenance+OR+servicing%29+%29+AND+NOT+%28searchableCompanyName%3A%28cancer+OR+Magnolias+OR+Warner+OR+ALSTOM+OR+Nokia+OR+IBM+OR+%22Bank+of+America%22+OR+JPMorgan+OR+Walmart+OR+HP+OR+AT&T+OR+At-tech+OR+Scipher+OR+%22Occupational+Health%22+OR+Sinai%29+%29/ws-75/sb-rd', '23226', '2', 'r', 'mediationjobs.jobamatic.com', $ipaddress);
//Query string notes: %22 is used to wrap around an exact phrase match e.g. %22business+coordinator%22.
// %3A is a colon.
// %28 is an open parenthesis (, and %29 is a close parenthesis ).
if ($response)
{
...
foreach($xmlObject->rs->r AS $key=>$value)
	...
	$jobdescription = $value->e; // Excerpt of job description
	...
	// Further perfect by ensuring that the first letter after a close parenthesis ')' gets an intervening space and is capitalized (it remains lower-case otherwise as in '(Volunteer)mediator'.
	$jobtitlearray = explode(')', $jobtitle); // Explode the string around the slash character.
	for ($count=0; $count < sizeof($jobtitlearray); $count++) // Loop for as many elements as we have in the array
		{
		$jobtitlearray[$count] = ucfirst($jobtitlearray[$count]); // Create an initial capital for each of the exploded particles (array elements).
	...
	$jobtitle = str_replace('Hah', 'HAH', $jobtitle);
	$jobtitle = str_replace('voip', 'VoIP', $jobtitle);
	$jobtitle = str_replace('Foia', 'FOIA', $jobtitle);
	$jobdescription= str_replace('Foia', 'FOIA', $jobdescription);
	$jobtitle = str_replace('Mba', 'MBA', $jobtitle);
	$jobtitle = str_replace('Adr', 'ADR', $jobtitle);
	$jobtitle = str_replace('And', '&amp;', $jobtitle);
	$jobtitle = str_replace(' Dc', ' DC', $jobtitle);
	$jobtitle = str_replace('Ip ', 'IP ', $jobtitle);
	$jobtitle = str_replace('Pi ', 'PI ', $jobtitle);
	$jobtitle = str_replace('Hr ', 'HR ', $jobtitle);
	...
	$jobtitle = str_replace('Volunteer Mediator (Free Training Provided)', 'Volunteer Mediator (Training Provided)', $jobtitle);
	$jobtitle = str_replace('Volunteer Mediator (F', 'Volunteer Mediator', $jobtitle);
	$jobtitle = str_replace('Counselor/Mediator Min', 'Counselor/Mediator', $jobtitle);
	...
	echo '<div class="dirlist"><span class="smallcaps">Mediation Job Location:</span> '.$joblocation.'</div>';
