...
if (isset($CheckBackLink))
	...
	if (!stristr($BackLinkPage, 'http')) $BackLinkPageURL = 'http://'.$BackLinkPage; else $BackLinkPageURL = $BackLinkPage;
...
// Remove trainer from trainers_table (and all his/her events from events_table) if he/she clicked the $DeleteTrainer check-box. Also remember to release the trainer's username-password pair for reuse by another trainer. Finally, exit script. And send appropriate messages to myself and to the departing trainer.
if (isset($DeleteTrainer))
	{
	unset($DeleteTrainer);
	
	$query = "SELECT * FROM trainers_table WHERE Username = '".$_SESSION['Username']."' AND Password = '".$_SESSION['Password']."'";
	$result = mysql_query($query) or die('The attempt to select a TrainerID from trainers_table has failed. ' . mysql_error());
	$row = mysql_fetch_assoc($result);
	// Specifically identify the TrainerID of the trainer for whom the user has logged in so we can use it when deleting this trainer's events from the events_table.
	$TrainerID = $row['TrainerID'];
	$theEntity = $row['EntityName']; // For use in the confirmation message displayed on the screen by edittrainer_slave.php.

	/* First send myself an email to remind myself to remind this trainer to cancel his/her subscription to NMTR. Need to capture his/her data before it's deleted from the DB. */
	$address = 'paul@mediationtrainings.org';  // When testing, I commented out this line, and uncommented the abertawe address below.
	$subject = "NMTR: $TrainerName1 Has Deleted His/Her Trainer Profile";
	$theRowData = '';
	...
	$body = "Hello\n\n";
	$body .= "This is an auto-generated message via edittrainer_slave.php to let you know that someone has deleted his/her Trainer Profile in NMTR. An auto-generated email was also sent to the TrainerEmail1 and PayerEmail address(es) to remind him/her/them to cancel their subscription. And the person who deleted the Trainer Profile would have seen an on-screen confirmation message that included an 'Unsubscribe' button after clicking the 'Delete My Profile' button. Even so, it's worth checking that the subscription has been cancelled in a few days. The details (from trainers_table, before the row was deleted) are as follows and have also been saved to log file deletedtrainers.txt:\n";
	$body .= $theRowData;
	$headers = 
...
"X-Mailer: PHP/" . phpversion();
	...
	// Also, as a backup, store details of each trainer who deletes himself/herself from the Registry in log file deletedtrainers.tx
	ob_start();
	echo date("D M j G:i:s T Y") . "\n"; 
	print_r($theRowData);
	$datadump = ob_get_clean();
	file_put_contents("/home/paulme6/public_html/medtrainings/logs/deletedtrainers.txt", $datadump, FILE_APPEND);
	
	// Send a farewell message to the trainer, reminding him/her to unsubscribe (i.e. via PayPal) to prevent future payments.
	$address = $row['PayerEmail'].','.$row['EntityEmail']; 
	$subject = "Remember to Cancel Your Subscription";
	$body = "Hello ".$row['PayerName']."\n\n";
	$body .= "I see that you've recently deleted your Trainer Profile from the National Mediation Training Registry. We're sorry to see you go and hope you'll rejoin soon! The Registry is the top-ranked site for mediation trainers and draws hundreds of unique visitors every day.\n\n";
	$body .= "Deleting your Trainer Profile doesn't automatically terminate your monthly subscription to the Registry. We can't do that for you. You have to do it yourself via the PayPal account that you used when you originally signed up. If you haven't already unsubscribed, visit our home page [http://www.mediationtrainings.org] and click the 'Unsubscribe' link at the bottom of the page.\n\n";
	$body .= "Thank you for your past support of the National Mediation Training Registry\n
Sincerely\n
Paul R. Merlyn
Administrator
...
"X-Mailer: PHP/" . phpversion();
	...
	<tr>
	<td style="text-align: left;">
	<p class='basictext' style='margin-left: 50px; margin-right: 50px; margin-top: 40px; font-size: 14px;'>You have successfully deleted <?=$theEntity; ?> from the National Mediation Training Registry.</p>
	<p class='basictext' style='margin-left: 50px; margin-right: 50px; font-size: 14px;'>Please don&rsquo;t forget to also unsubscribe from the Registry to cancel future payments. (We can&rsquo;t unsubscribe for you.) You have to do it yourself via the PayPal account that you used when you originally signed up.</p>
	<p class='basictext' style='margin-left: 50px; margin-right: 50px; margin-bottom: 40px; font-size: 14px;'>If you haven&rsquo;t already unsubscribed, click the button below to initiate that process.</p>
	<p class='basictext' style='margin-left: auto; margin-right: auto; text-align: center; margin-bottom: 40px; font-size: 14px;'>
	<A HREF="https://www.paypal.com/cgi-bin/webscr?cmd=_subscr-find&alias=ZGV4J85ZEA8B8"><IMG BORDER="0" SRC="https://www.paypal.com/en_US/i/btn/btn_unsubscribe_LG.gif" alt="PayPal - Unsubscribe"></A>
	</p>
	</td>
	</table>
	</form>
...
else $BackLinkPageURL = $BackLinkPage;
$ClaimedBackLinkPageContents = @file_get_contents($BackLinkPageURL); // $ClaimedBackLinkPageContents stores the contents of the URL (i.e. $BackLinkPage) that the trainer claims will contain a link back to the NMTR web site. Note that the "@" symbol suppresses a "Couldn't resolve host name" PHP warning message that would appear when the file_get_contents() function is called on an illegitmate URL.
if (!stristr($ClaimedBackLinkPageContents, 'mediationtrainings.org')) 
	{
		$BackLinkPage = NULL;
